...
> **Status:** `0.12.1` — chat-assisted global outline + automatic structure generation + orphan scene panel + accessible consistent buttons + partial outline edits (update_outline) + beats from the author's manuscript via chat. Local-first fiction co-writer where **the conversation is the product**: an agent you converse with that knows the manuscript, bible, outline and compass, and that **applies** changes to the content when you accept them.
